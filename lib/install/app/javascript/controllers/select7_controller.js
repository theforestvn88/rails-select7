...
export default class extends Controller {
    ...
    localSuggest() {
        ...
        if (key != "") {
            this.suggestionTarget.childNodes.forEach(node => {
                if (node.nodeType == Node.ELEMENT_NODE) {
                    node.classList.remove("select7-hidden")
                    if (node.id.toLowerCase().indexOf(key) == -1) {
                        node.classList.add("select7-hidden")
                    }
                }
            })
